"""Phase-aware LTD training launcher for the **depth** policy on Qwen3 (and
others). This launcher does *not* modify ``rl/rl_depth.py``; it imports the
existing ``SpeculativeDecodingEnv`` and bolts a ``PhaseObsWrapper`` on top.

V1 limitation: trains depth alone with a fixed inner verification size (V=60),
matching how the paper trains its initial depth policy. Co-adaptive iterative
training with a phase-aware size policy is deferred to a follow-up.

Example:
    python3 -m rl.rl_depth_phase \\
        --base_model_path Qwen/Qwen3-8B \\
        --ea_model_path <eagle3-qwen3-path> \\
        --dataset_train gsm8k \\
        --total_timesteps 1000000 \\
        --use_phase --reward_mode cycle
"""
...
# Ensure the project root is on sys.path so `rl.*` and `eagle.*` resolve.
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Phase-aware LTD: depth policy")
    # Paths
    ...
    p.add_argument("--qwen3_mode", action="store_true", default=True,
                   help="Use Qwen3 <think>/</think> token IDs to gate reasoning phase.")
    ...
    p.add_argument(
        "--wandb_project", type=str, default="speculative-decoding-rl-phase",
...
# ---------------------------------------------------------------------------
# Callback (mirrors rl/rl_depth.py but logs phase distribution too)
# ---------------------------------------------------------------------------
    ...
    def _on_step(self) -> bool:
        ...
        for info in infos:
            ...
            for k_src, k_dst in (
                ...
                ("base_reward", "custom/base_reward"),
                ...
                ("depth_chosen_action", "custom/depth_chosen_action"),
...
def main() -> None:
    ...
    ckpt_dir = os.path.join(args.save_path, model_name, "depth_phase")
    ...
    # ---- env (base + phase wrapper) ----
    DepthEnvBase, _ = load_base_env_classes()
    env = DepthEnvBase(model, logits_processor=None, input_ids_list=input_ids_list)
    if args.use_phase:
