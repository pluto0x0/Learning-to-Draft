...
# Phase-aware LTD: depth policy training launcher.
# Mirrors train_depth.sh but invokes rl/rl_depth_phase.py and adds phase flags.

# ── Defaults (Qwen3-8B reasoning) ─────────────────────────────────────────────
gpu=0
...
usage() {
    cat <<EOF
Usage: bash train_depth_phase.sh [OPTIONS]

Phase-aware extension flags:
  --use_phase            Enable phase obs + reward shaping (default: on)
  --no_phase             Disable phase (recovers original LTD)
  --reward_mode          cycle|blend|segment (default: $reward_mode)
  --blend_alpha          Blend weight for cycle reward (default: $blend_alpha)
  --phase_smoothing      Required consecutive same-phase cycles to flip (default: $phase_smoothing)
  --qwen3_mode           Use <think>/</think> token IDs (default: on)
  --no_qwen3_mode        Treat all generation as in-think
EOF
}
...
if [[ -z "$ea_model_path" ]]; then
    echo "ERROR: --ea_model_path is required (Eagle3 draft model for the chosen base)." >&2
    exit 1
fi
